...
for test_index in range(8):
    ...
    tf.keras.models.save_model(clf.model, f"./data/saved_models/hybrid_{test_sample}.h5")
...
#    y_prob = saved_model.predict(x=[X_test_sigs, X_test_seqs])
    np.save('/home/ec2-user/files/%s_y_prob.npy' % test_sample, y_prob)
    y_pred = [int(np.round(x)) for x in y_prob]
    np.save('/home/ec2-user/files/%s_y_test.npy' % test_sample, y_test)
    np.save('/home/ec2-user/files/%s_y_pred.npy' % test_sample, y_pred)
    elapsed_time = time() - start_time
    ...
    np.save('/home/ec2-user/files/%s_fpr.npy' % test_sample, fpr)
    np.save('/home/ec2-user/files/%s_tpr.npy' % test_sample, tpr)
    np.save('/home/ec2-user/files/%s_thresholds.npy' % test_sample, thresholds)

    logging.debug("AUC: {}".format(auc(fpr, tpr)))
    np.save('/home/ec2-user/files/%s_auc.npy' % test_sample, auc)
...
#    y_pred_tss = [int(np.round(x)) for x in y_prob_tss]
    ...
    np.save('/home/ec2-user/files/tss_%s_fpr.npy' % test_sample, fpr)
    np.save('/home/ec2-user/files/tss_%s_tpr.npy' % test_sample, tpr)
    np.save('/home/ec2-user/files/tss_%s_thresholds.npy' % test_sample, thresholds)
    np.save('/home/ec2-user/files/tss_%s_y_pred.npy' % test_sample, y_pred_tss)
    np.save('/home/ec2-user/files/tss_%s_y_prob.npy' % test_sample, y_prob_tss)
    np.save('/home/ec2-user/files/tss_%s_y_test.npy' % test_sample, y_test_tss)

    logging.debug("AUC: {}".format(auc(fpr, tpr)))
    np.save('/home/ec2-user/files/%s_auc.npy' % test_sample, auc(fpr, tpr))
...
#    y_pred_notss = [int(np.round(x)) for x in y_prob_notss]
    ...
    np.save('/home/ec2-user/files/notss_%s_fpr.npy' % test_sample, fpr)
    np.save('/home/ec2-user/files/notss_%s_tpr.npy' % test_sample, tpr)
    np.save('/home/ec2-user/files/notss_%s_thresholds.npy' % test_sample, thresholds)
    np.save('/home/ec2-user/files/notss_%s_y_pred.npy' % test_sample, y_pred_notss)
    np.save('/home/ec2-user/files/notss_%s_y_prob.npy' % test_sample, y_prob_notss)
    np.save('/home/ec2-user/files/notss_%s_y_test.npy' % test_sample, y_test_notss)
